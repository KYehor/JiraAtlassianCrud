...
public class IssueCRUD extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        ...
        switch (action) {
            ...
            case "edit":
                ...
                break;
            default:
                List<String> issues = getRepeatedWords();
    ...
    private List<String> getRepeatedWords() {
        List<Issue> issues = getIssues();
        List<String> list = new ArrayList<String>();
        List<String> repeatedWord = new ArrayList<String>();
        Set<String> set = new HashSet<>();

        for (Issue issue : issues) {
            repeatedWord.add(issue.getSummary());
            repeatedWord.add(issue.getDescription());
        }

        StringBuffer sb = new StringBuffer();
        for(int i = 0; i < repeatedWord.size(); i++) {
            sb.append(repeatedWord.get(i)+" ");
        }
        String finalRepeatedWords = sb.toString();
        String result = finalRepeatedWords.replaceAll("[\\d\\-\\+\\.\\^:,]", "");
        repeatedWord = Arrays.asList(result.split(" "));

        // Another way through regex are commented below
        /*String duplicatePattern = "(?i)\\b(\\w+)\\b[\\w\\W]*\\b\\1\\b";
        Pattern p = Pattern.compile(duplicatePattern);
        Matcher m = p.matcher(result);
        while (m.find()) {
            String finalResult = m.group(1) + m.group(0).replaceAll(m.group(1),"\n");
            list.add(String.join(" ",finalResult.split(" ")));
        }*/

        //Used HashSet to find unique repeated words
        for (String s : repeatedWord) {
            if (!set.add(s)) {
                list.add(s);
            }
        }
